...
static int rdf3xload_serial(const char *database, const char *input)
{
   // Parse the input
	TempFile rawFacts(database),rawStrings(database);
	...
	if (!parse(in,input,lookup,rawFacts,rawStrings,subTypes))
		return 1;

   // Build the string dictionary
   TempFile stringTable(database),stringIds(database);
   buildDictionary(rawStrings,stringTable,stringIds,subTypes);

   // Resolve the ids
   TempFile facts(database);
   resolveIds(rawFacts,stringIds,facts);
   stringIds.discard();

   // And start the load
   loadDatabase(database,facts,stringTable);
